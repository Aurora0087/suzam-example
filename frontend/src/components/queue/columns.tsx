...
export const queueColumns: ColumnDef<QueueItem>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      const statusConfig = {
        ...
        downloading: { icon: Download, color: "text-blue-500 animate-bounce", label: "Downloading" },
        fingerprinting: { icon: Cpu, color: "text-purple-500 animate-pulse", label: "Analyzing" },
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const errMsg = typeof row.original.err_message === 'object' 
  ...
  },
  {
    ...
    header: "Started",
    cell: ({ row }) => {
      // Formats the SQLite timestamp (e.g., 2023-10-27 10:00:00)
